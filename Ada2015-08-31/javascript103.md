This week we're making a clone of the game 2048. The code is open source, so many people have altered and added functionality over time... But we'll just make the base game. :) To do this, we need to learn more about javascript code organization.
...
- .break - While inputting a multi-line expression, sometimes you get lost or just don't care about completing it. .break will start over. ctrl + C
- .clear - Resets the context object to an empty object and clears any multi-line expression.
- .exit - Close the I/O stream, which will cause the REPL to exit. Close node. Can also use ctrl + D
...
In other words, inner function refers to parent scope and needs to keep vars defined there around.
...
```ruby
class Powerer
  # Serves a role similar to a closure. Requires a value, uses instance variable to hold onto it so exper knows what to do.
...
```
In ruby, lamdas are used for actual closures... but this does something similar.  
...
When to consider using a closure:
  - when you wish you had an instance var
  - when you have a complex parallel if/else series or case/switch.
